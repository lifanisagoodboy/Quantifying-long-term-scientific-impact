...
# 将文本汇总成一个txt
def get_text():
    base_path = "E:/code/Quantifying long-term scientific impact code/3_Text_Cluster\data_offline/"
...
def trans_text(item):
    base_path = "E:/code/Quantifying long-term scientific impact code/3_Text_Cluster/data_temp2/%s/" % item
    filelist = os.listdir(base_path)
    ...
    for files in filelist:
        ...
        f2 = open("E:/code/Quantifying long-term scientific impact code/3_Text_Cluster/data_test/" + salt + '.txt', 'w')
        f2.write(text)
        f3.write(salt + ' ' + item + '\n')  # 添加类别
        f.close()
        f2.close()
...
def trans_res():
    ...
    for (k, v) in res:
        if str(v) == '1':
            fff1 = open("E:/code/Quantifying long-term scientific impact code/3_Text_Cluster/data/%s.txt" % k, 'r')
            text = fff1.read()
            data_temp = text.decode('utf-8')  # 转换为unicode编码形式
            data = ''.join(re.findall(u'[\u4e00-\u9fff]+', data_temp))  # 必须为unicode类型，取出所有中文字符
            fff1.close()
            ...
            # text_list1.append(k)
        if str(v) == '2':
            fff2 = open("E:/code/Quantifying long-term scientific impact code/3_Text_Cluster/data/%s.txt" % k, 'r')
            text = fff2.read()
            data_temp = text.decode('utf-8')  # 转换为unicode编码形式
            data = ''.join(re.findall(u'[\u4e00-\u9fff]+', data_temp))  # 必须为unicode类型，取出所有中文字符
            fff2.close()
            ...
            # text_list2.append(k)
        if str(v) == '0':
            fff0 = open("E:/code/Quantifying long-term scientific impact code/3_Text_Cluster/data/%s.txt" % k, 'r')
            text = fff0.read()
            data_temp = text.decode('utf-8')  # 转换为unicode编码形式
            data = ''.join(re.findall(u'[\u4e00-\u9fff]+', data_temp))  # 必须为unicode类型，取出所有中文字符
            fff0.close()
